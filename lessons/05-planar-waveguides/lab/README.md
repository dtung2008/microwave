# Lecture 5 lab — Planar lines & waveguides
...
Optional, for hour 3's below-cutoff field demo (the lab runs without it):
...
| `hw5_starter.py` | the only code file you edit and submit |
| `VERIFY.md` | instructor-side verification recipe |
| `solutions/` | instructor only |

Generated when you run things (all regenerable, none submitted):
`hw5_sweep.png`, `microstrip_referee.png`, `omega_beta.png`, `fdtd_cutoff.png`,
`openems_case.png`, `PLACEHOLDER_mline.s2p`.

**About `openems_microstrip.s2p`:** hour 3's cell 3.6 post-processes the
instructor's openEMS (full-wave field solver) export of the course's 50 Ω line.
That file is distributed in class, not in the repo. Until you drop it into this
directory, the cell builds and loudly labels a scikit-rf placeholder so the
post-processing pipeline can still be rehearsed — the code path is identical.
...
- **`fdtd` missing** — cell 3.5 prints a note and skips; everything else runs.
  `pip install fdtd` (pure Python + numpy; no compiler needed).
