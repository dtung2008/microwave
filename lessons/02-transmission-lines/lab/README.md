# Lecture 2 lab — Transmission-line theory

## Install

Same environment as lecture 1 — nothing new to install. If you set up for
lecture 1, just verify:

```
python setup_check.py           # must print SETUP OK
```

If you are joining the course here: **Python 3.12, exactly** (the course pins
`numpy==1.26.4`, which has no wheels for Python 3.13+). From the repo root
(all OSes — Windows shown; use `python3.12` on macOS/Linux):
...
| `hw2_starter.py` | the only code file you edit and submit |
...
- **`z_in` returns `inf`/`nan` at exactly the quarter-wave point** — you used the
  lossless `tan` form and hit its pole. Use the lossy `tanh` form (it is the
  general one); `np.tanh` stays finite where `np.tan` explodes.
