# Lecture 8 lab — Filters I: the insertion-loss method
...
| `hw8_starter.py` | the only code file you edit and submit |
...
- **`arccosh` returns `nan` in `min_order`** — your mapped Ω_s came out below 1.
  Check the sign and the ordering: the map Ω(f) is negative below f₀; take |Ω|
  before the arccosh, and make sure Δ uses f₀ = √(f₁f₂).
- **Group delay looks spiky or jumps by microseconds** — the phase must be
  *unwrapped* before differentiating (`np.unwrap`), and the sweep grid must be
  dense enough; use the toolkit's `group_delay_s` on `F_SWEEP_HZ`.
- **Your ripple reads 0.5001 dB or the margin prints −0.0000** — that is the
  equal-ripple design spending its entire budget: the band-edge attenuation *is*
  the ripple for odd N. Anything beyond ~1e-3 dB over, though, means a real bug
  (check every branch resonates at 59.7913 MHz, not 60).
